...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const assertSessions = async (jids: string[], force?: boolean) => {
		...
		// Upstream #2432 fix: removed `peerSessionsCache` — its 'session exists' TTL outlived
		// session deletions caused by identity changes / Bad MAC, so a stale `true` would
		// short-circuit assertSessions and downstream encrypt would fail silently. Always
		// hit `validateSession` (cheap libsignal read) and let the result drive the decision.
	...
	const relayMessage = async (
		...
		const destinationJid = !isStatus ? resolveDirectRecipientWireJid(finalJid, directRecipient?.lidJid) : statusJid
		...
		const runSendBody = async () => {
			...
			if (!isRetryResend) {
				emitMessageDeliveryState(ev, {
					key: { remoteJid: jidNormalizedUser(jid), fromMe: true, id: msgId },
					state: 'accepted',
					requestedJid: jidNormalizedUser(jid),
