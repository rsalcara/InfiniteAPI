/**
 * Schema for `msgstore.db` — message routing, device cache, retry counters,
 * and quarantine storage.
 *
 * Gateway scope (only tables that solve operational pain points):
 *
 *   - `jid` + `jid_map` — LID↔PN bidirectional mapping; both addressing
 *     forms become rows in `jid` with `jid_map` as the lookup table
 *     (target for phase 9.1 `LIDMappingStore` migration)
 ...
 *     with UNIQUE on the natural retry-dedup key (phase 9.3)
