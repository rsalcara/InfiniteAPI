...
/**
 * The 11 physical SQLite files we open in multi-DB mode, one per concern:
 *
 ...
 *                              MULTI_DB_FILES list.
 *
 * Discontinued (removed — no backend/consumer, schema-only, not relevant to
 * InfiniteAPI): `payments.db` (payment state), `smb.db` (Small Business /
 * Marketing Messages), and earlier `prometheus.db` (metrics history). Sessions
 * that already have those files on disk simply leave them untouched — they are
 * no longer opened or created.
 */
