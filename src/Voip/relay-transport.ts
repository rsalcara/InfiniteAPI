...
export class RelayRtcTransport {
	...
	send = (packet: Uint8Array | Buffer, ip: string, port: number): number => {
		...
		if (!info) {
			...
			bufferPacket(earlyConnection, toArrayBuffer(packet), this.#totals)
	...
	#loadWrtc = (): Promise<any> => {
		this.#wrtcPromise ??= import('@roamhq/wrtc').then(module => (module.default ?? module) as any)
